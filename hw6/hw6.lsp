...
;
;
;NOTE: added an extra arg to make it work
(defun at-most-one-color (n c k absk)
	(cond	((= c k) nil)
		((< c k) (append (list (list (- 0 (node2var n c absk)) (- 0 (node2var n k absk))))
				 (at-most-one-color n c (- k 1) absk)
				 (at-most-one-color n (+ c 1) k absk)
			 )
		)
	)
)
...
(defun generate-node-clauses (n k)
	(append (list (at-least-one-color n 1 k)) (at-most-one-color n 1 k k))
)
...
(defun generate-edge-clauses (e k)
	(append (generate-node-clauses (first e) k) (generate-node-clauses (second e) k))
)
